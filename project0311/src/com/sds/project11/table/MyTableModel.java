package com.sds.project11.table;
...
public class MyTableModel extends AbstractTableModel{
	//데이터가 되는 이차원 배열
	//자바에서 배열은 길이가 고정되므로, 데이터가 추후 지속적인 변경사항이 발생한다면 배열보다는 보다 
	//유연한 컬렉션 프레임웍을 이용해보자
	// - 컬렉션 프레임원? java에서 객체만을 모아서 처리할때 유용한 기능을 지원하는 패키지
	// 이패키지 안에는 여러가지 객체들이 지원되는데, 3가지가 있고 전부 인터페이스이다
	// 1) List 형 - 순서있는 집합을 표현한 객체 
	//				   배열과 거의 같지만, 길이가 유동적이고 오직 객체만을 담을 수 있다.
	// 2) Set 형 - 순서 없는 집합을 표현한 객체
	// 3) Map 형 - key-Value의 쌍으로 집합을 표현한 객체
	
	//배열을 대체할 보다 유연한 컬렉션 ㅍ ㅡ레임원의 List의 자식중 하나인 ArrayList를 사용해본다
	...
	public String getColumnName(int col) {
		...
		System.out.println(col+"번째 컬럼이름");
		return column[col];
	}
	...
	public Object getValueAt(int row, int col) {
		...
		String[] member = (String[]) obj; //object형태로 들어가 있던 일차원 배열을 다시 형변환해서 복원
